...
# This python 2.7 script will allow older Mac computers to run BOINC at the highest CPU load possible without spinning up the computer fan. The fan spins up when the CPU gets too hot. At higher spin rates I find the fan to be annoyingly loud.
...
# define a function that can edit the BOINC global preferences file and ask BOINC to use this updated file.
def adjustcpuusage(cpudelta):
	...
	s = str(data) # convert data to a string so I can more easily manipulate it
	...
	if newcpuusageinteger < cpuusagelowerlimit:
		...
		print "Updated CPU usage of " + str(newcpuusageinteger) + " percent is between limits of " + str(cpuusagelowerlimit) + " percent and " + str(cpuusagedupperlimit) + " percent."
	#print "CPU usage is now " + str(newcpuusageinteger) + " percent."

	s6 = str(newcpuusageinteger) + ".000000" # add the appropriate number of 0's back to it. newcpuusageinteger may be 1, 2, or 3 digits in length.
	...
	os.chdir(owd) # this will bring us back to the original working directory defined above.

# function ends.


# determine the current fan speed of the mac using utility iStats,
...
while True:
	...
	# determine if current fan speed is greater than speed limit I have imposed (to keep fan noise down). use the function if necessary.
	speedlimit = 2250 # assign a speed limit for the fan. we will adjust the boinc processur usage to make sure this speed limit is not exceeded.
	if currentspeed > (speedlimit + 3800):
		...
		adjustcpuusage(-5)  # reduce CPU load by 5 percentage points (but not below lower limit defined in function)
		...
		adjustcpuusage(-4)  # reduce CPU load by 4 percentage points (but not below lower limit defined in function)
		...
		#adjustcpuusage(1)  # increase CPU load by 1 percentage point (but not above upper limit defined in function)
